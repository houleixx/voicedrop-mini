...
test('page header back button stays left aligned', () => {
  ...
  assert.match(body, /left:\s*calc\(32rpx - 5px\);/)
  assert.match(body, /bottom:\s*calc\(24rpx - 6px\);/)
...
test('page header back icon is centered inside the button', () => {
  ...
  const surface = ruleBody(css, '.header-back-surface')

  assert.doesNotMatch(wxml, />‹<\/text>/)
  assert.match(wxml, /class="header-back-surface">\s*<text class="header-back-icon header-back-icon-arrow"><\/text>/)
  assert.doesNotMatch(wxml, /&#xea60;/)
  assert.doesNotMatch(icon, /margin-top:\s*-/)
  assert.match(surface, /width:\s*32px;/)
  assert.match(surface, /height:\s*32px;/)
  assert.match(surface, /border:\s*1rpx solid rgba\(224, 216, 204, 0\.72\);/)
  assert.match(surface, /background:\s*#fffdf9;/)
  assert.match(surface, /box-shadow:\s*0 6rpx 18rpx rgba\(65, 52, 41, 0\.1\);/)
  assert.match(icon, /width:\s*32px;/)
  assert.match(icon, /height:\s*32px;/)
  assert.match(icon, /font-family:\s*'remixicon'\s*!important;/)
  assert.match(icon, /font-size:\s*20px;/)
  assert.match(icon, /font-weight:\s*700;/)
  assert.match(icon, /line-height:\s*32px;/)
