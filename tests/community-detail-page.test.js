...
test('community detail has custom actions and loading markup', () => {
  ...
  assert.match(wxml, /<view class="loading-card" wx:if="\{\{loading\}\}">\s*<view class="loading-spinner" aria-hidden="true"><\/view>\s*<text>\{\{i18n\["加载中\.\.\."\]\}\}<\/text>\s*<\/view>/)
  ...
  assert.match(wxml, /aria-label="\{\{i18n\['投币'\]\}\}"/)
...
test('community article detail shares the list gutter and audio-detail reading rhythm', () => {
  ...
  assert.match(wxss, /\.article-meta\s*\{[^}]*margin-top:\s*10px;[^}]*gap:\s*16rpx;[^}]*font-size:\s*26rpx;[^}]*line-height:\s*1\.4;/s)
