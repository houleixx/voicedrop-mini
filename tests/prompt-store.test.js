...
test('uses exact restore preview import and share endpoints', async () => {
  ...
  d.queue.push({ statusCode: 200, data: { schema: 1, items: [{ id: 'g_shared', type: 'group', label: '共享组', origin: 'user', children: [{ id: 'p_12345678', type: 'action', label: '共享', origin: 'user', prompt: 'P', appliesTo: ['text'] }] }] } })
  assert.equal((await store.importCode('1234567')).ok, true)
  assert.equal(d.calls.at(-2).url, 'https://example.test/agent/prompts/import')
  assert.equal(d.calls.at(-1).url, 'https://example.test/agent/prompts')
  assert.equal(store.items()[0].children[0].id, 'p_12345678')
