...
test('prompt list markup contains the screenshot hierarchy and import copy', () => {
  ...
  assert.match(header, /slot="left"[\s\S]*wx:if="\{\{reordering\}\}"[\s\S]*bindtap="cancelReorder"[\s\S]*>\{\{i18n\["取消"\]\}\}</)
  assert.doesNotMatch(header, />取消排序</)
  assert.match(header, /slot="right"[\s\S]*>\{\{i18n\["完成"\]\}\}</)
  assert.doesNotMatch(wxml.replace(header, ''), /bindtap="cancelReorder"/)
  assert.match(wxml, /slot="right"[\s\S]*class="add-button"/)
  assert.match(wxml, /i18n\["输入魔法数字导入"\]/)
  assert.match(wxml, /class="market-heading">\{\{i18n\["社区提示词"\]\}\}</)
  assert.match(wxml, /class="market-filters"/)
  assert.match(wxml, /bindtap="selectMarketFilter"/)
  assert.doesNotMatch(wxml, /社区热门/)
  assert.match(wxml, /class="prompt-state prompt-loading-state"[^>]*>[\s\S]*class="prompt-loading-spinner"[\s\S]*i18n\["正在加载提示词…"\]/)
  assert.match(wxml, /class="market-state market-loading-state"[^>]*>[\s\S]*class="prompt-loading-spinner"[\s\S]*i18n\["正在加载社区提示词…"\]/)
  assert.match(wxml, /market-detail-stats/)
  assert.match(wxml, /i18n\["被导入"\]/)
  assert.match(wxml, /i18n\["适用于"\]/)
  assert.match(wxml, /i18n\["效果示例"\]/)
  ...
  assert.match(wxml, /originKey === '导入' \? 'imported-origin'/)
  assert.match(wxml, /originKey === '已自定义' \? 'custom-origin'/)
  ...
  assert.match(wxml, /placeholder="\{\{i18n\['分组名字'\]\}\}"/)
