...
Page({
  data: { type: 'action', pageTitle: '提示词', label: '', prompt: '', text: true, image: true, saving: false, error: '' },
  onLoad(options) { const type = options && options.type === 'group' ? 'group' : 'action'; this.setData({ type, pageTitle: type === 'group' ? '新建分组' : '提示词' }) },
  ...
  toggleText() { this.setData({ text: !this.data.text }) },
  toggleImage() { this.setData({ image: !this.data.image }) },
  async save() {
