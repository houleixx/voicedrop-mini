...
5. Community sort tabs use explicit `88rpx` item heights and a fixed label line box. A label-only optical correction outside real iOS runtimes accounts for the visible Chinese glyph box sitting above the font line box without changing the tab hit area: `9rpx` by default and `14rpx` on HarmonyOS.
...
8. VD community feed filters use a native `text` with a fixed `30rpx` line box inside an `88rpx` Flex hit target. Because WeChat Developer Tools, Android, HarmonyOS, and potentially unknown runtimes expose a visibly high glyph baseline, a runtime-gated `9rpx` correction is enabled by default and disabled only for a positively identified real iOS runtime. HarmonyOS receives `14rpx` because fresh device screenshots show its visible glyph center remains about `5rpx` higher than Android after the default correction.
