...
|---|---|---|
| Networking platform header | Implemented with `X-VD-Platform: miniapp` across request, upload, download, and WebSocket entry points | `services/request`, `services/audio`, `services/library`, `services/article-edit`, `services/library-command`, `services/status-session` |
...
| Generate article from audio file | Not implemented; the Mini Program keeps its native recording flow instead of an Android/iOS-style share receiver | `pages/record`, `services/audio` |
...
| Share to Xiaohongshu | Implemented as a Mini Program-compatible export: request the shared content pack, copy its text, save original photos and local 3:4 text cards to the album, then guide manual posting. A Mini Program cannot hand these assets into Xiaohongshu's native composer. | `services/xhs-export`, `utils/xhs-cards`, `utils/album-permission`, `pages/detail` |
| Voice article edit | Partially substituted: text commands and full edit WebSocket are implemented; long-press menu `itemId` survives the durable queue and is sent for generated-image XMP provenance; Android raw PCM ASR is platform-limited | `services/article-edit`, `pages/detail` |
...
| Long-press edit menus | Implemented with prompt-tree menus; the selected leaf id, structured anchor, and filled instruction are sent together | `utils/prompt-tree`, `components/config-menu`, `pages/detail` |
| Shared prompt group placement | Implemented: after a successful import the full prompt tree is refreshed so server-side `groupPath` placement is visible immediately; failed refreshes preserve the successful local import fallback | `services/prompt-store`, `pages/instruction-settings`, `pages/community-detail` |
...
| Generate article from images | Not implemented as a standalone share receiver; images can still be attached during the Mini Program recording and article-edit flows | `pages/record`, `pages/detail` |
...
| Style dataset collection and extraction | Not exposed by the Mini Program; writing styles remain manually editable with version history | `pages/style-settings`, `services/settings` |
...
| WeChat official account drafts | Implemented: Android-compatible third-party authorization via `/wechat/authorization`, connection checks via `/wechat/bind-status`, `/wechat/unbind`, signed authorization link copied to an external browser, publish/update preflight and `/wechat/{articleKey}` draft endpoint | `pages/wechat-settings`, `services/settings`, `services/library.publishWechat` |
