...
| WeChat official account drafts | Implemented: strict local credential format checks, authenticated pre-save `wechat-validate` through the whitelisted relay, tested console/QR/IP-whitelist guidance, publish/update flow, Chinese error mapping, and config-error routing | `pages/wechat-settings`, `services/settings`, `services/library.publishWechat` |
